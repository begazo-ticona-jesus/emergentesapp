...
edge_host = "192.168.5.199"  # Este es el host de tu entorno local de edge computing
...
def on_message_from_iot(client, userdata, message):
    payload_variable = message.payload.decode('utf-8')
    print(f"Mensaje recibido del ESP8266 en el tópico {message.topic}: {payload_variable}")
    format_data = {"motion": 0,"light": int(payload_variable)}
